...
public class CDataBase {
	...
	public ArrayList<CNoticia> getNoticias(){
		...
		try{
			String sql="SELECT noti.idnoticia, noti.titulo noti_titulo, noti.descripcion noti_descripcion,descripcion_corta, noti.areaidarea idarea, ifnull(noti.fecha_inicio,now())  fecha_inicio, "+
" ifnull(noti.fecha_fin,now()) fecha_fin, noti.prioridad prioridad, a.nombre nombre_area, m.idmultimedia idmultimedia, m.direccion direccion_m, m.direccion_relativa direccion_rel, m2.idmultimedia idmultimedia2, m2.direccion direccion_m2, m2.direccion_relativa direccion_rel2, noti.estado  "+
  ...
  "  where estado = 1 and fecha_inicio<=now() and adddate(fecha_fin,1)>=now()  and fecha_inicio>0 and fecha_fin>0 order by prioridad desc ";
			...
			while(rs.next()){
				...
				CNoticia news=new CNoticia(rs.getString("noti_titulo"),
						...
						new java.util.Date(rs.getDate("fecha_inicio").getTime()),
						new java.util.Date(rs.getDate("fecha_fin").getTime()),
						rs.getInt("prioridad"),rs.getInt("idnoticia"),
						rs.getInt("estado"),area,multi2);
				ret.add(news);
...
public int getResponsableTotal(int type,String busqueda){
  ...
  " inner join edificio edi on edi.idedificio=act.edificioidedificio where idactividad=?  ";
	...
	public boolean UpdateConfiguracion(CConfiguracion confi){
		PreparedStatement stm;
		try {
			stm = (PreparedStatement)conn.prepareStatement("UPDATE configuracion SET telefono = ?,  fax = ?, tamanio_sub = ?, direccion = ?, ciclo = ?, multifa_reporte = ?, random_carne = ?,  impresion_salud = ?,  jefe_actual = ?,  fecha_examen = ?, dependencia=?, no_personal=?, password=?, correo_javamail=?, pass_javamail=?, smtp_javamail=?, port_javamail=? , examen_linea=? WHERE   idconfiguracion = ?");
			...
			stm.setInt(7, confi.getRandom_carne());
			...
			stm.setInt(18, confi.getExamen_linea());
			stm.setInt(19, confi.getidconfiguracion());
