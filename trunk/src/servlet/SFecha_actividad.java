					 ...
					 CUsuarioPermiso user_permiso=(CUsuarioPermiso)sessiones.getAttribute("user_permiso");
								...
								CDataBase dbo=new CDataBase();
								 ...
								 if(action.equalsIgnoreCase("guardar_rango")&& (user_permiso.getIdpermiso().indexOf(229)>-1  || user_permiso.getIdusuario().getidusuario()==1)){
									...
									fecha_inicio=valid.Limpiarvalor(base64.decodificar(fecha_inicio));
									String fecha_fin=(valid.ValidarRequest(request.getParameter("fecha_fin")));		
									fecha_fin=valid.Limpiarvalor(base64.decodificar(fecha_fin));
									String hora_inicio=(valid.ValidarRequest(request.getParameter("hora_inicio")));		
									hora_inicio=valid.Limpiarvalor(base64.decodificar(hora_inicio));
									String hora_fin=(valid.ValidarRequest(request.getParameter("hora_fin")));		
									hora_fin=valid.Limpiarvalor(base64.decodificar(hora_fin));
									String phora_inicio=valid.Limpiarvalor(valid.ValidarRequest(request.getParameter("hora_inicio_mm")));
									String phora_fin=valid.Limpiarvalor(valid.ValidarRequest(request.getParameter("hora_fin_mm")));
									...
									out.println(result);
									 ...
									 int idactividad=valid.ConvertEntero(valid.ValidarRequest(request.getParameter("idactividad")));
										...
										fecha=valid.Limpiarvalor(base64.decodificar(fecha));
										String hora_inicio=(valid.ValidarRequest(request.getParameter("hora_inicio")));		
										hora_inicio=valid.Limpiarvalor(base64.decodificar(hora_inicio));
										String hora_fin=(valid.ValidarRequest(request.getParameter("hora_fin")));		
										hora_fin=valid.Limpiarvalor(base64.decodificar(hora_fin));
